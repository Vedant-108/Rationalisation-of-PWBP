...
def next_screen():
    ...
    # Ensure 'photo' is global so it persists
...
def gui_test2():
    ...
    # Create main window
    root_t2 = tk.Tk()
    root_t2.title("Design of Protective Water Barrier Pillar (PWBP)")
    # root.attributes('-fullscreen', True)  # Run in full screen
    # Set window size and position to fullscreen with 1920x1080 resolution
    # root.attributes('-fullscreen', True)
    root_t2.geometry("1366x768")
    #root.geometry("1920x1080")
    ...
    # Heading
    heading_label = tk.Label(root_t2, text="Estimates of Critical Width, Controlled Seepage Width,"
                                        " and Rational Width & Desired Water Head",
                             font=("Arial", 20, "bold"), pady=10)
    heading_label.pack()

    graph_frame = ttk.Frame(root_t2, width=1000, height=450)
    ...
    # Set the window state to maximized (zoomed)
    root_t2.wm_state('zoomed')

    # Table Frame
    table_frame = ttk.Frame(root_t2)
    table_frame.pack()
    Wc = round(Wc, 2)
    Wsp = round(Wsp, 2)
    Wr = round(Wr, 2)
    ...
    tooltip_label = tk.Label(root_t2, textvariable=tooltip_text, font=("Arial", 8, "bold"), bg="yellow", relief="solid",
                             borderwidth=1)
    tooltip_label.place_forget()

    # Create a tooltip label
    tooltip_text = tk.StringVar()
    tooltip_label = tk.Label(root_t2, textvariable=tooltip_text, font=("Arial", 8, "bold"),
    ...
    # Exit Button
    exit_button = tk.Button(root_t2, text="Exit", command=exit_app, width=20, height=2)
    exit_button.pack(side="right", padx=10, pady=10)  # Adjust padx and pady as needed

    # Plot Graph Button
    graph_button = tk.Button(root_t2, text="Plot Graph", command=plot_graph, width=20, height=2)
